// One-line note about the place currently on the map, written by an LLM.
//
...
//
// Requires OPENAI_API_KEY. Without it the endpoint returns an empty note and
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!process.env.OPENAI_API_KEY) return sendJson(res, 200, { note: "" }, 3600);
    ...
    const completion = await client.chat.completions.create({
      ...
      messages: [
        ...
        { role: "user", content: `${name}${coordinates}` },
      ],
    });
    ...
    return sendJson(res, 200, { note }, note ? 2592000 : 0);
  } catch (error) {
